    ...
    <div className="p-6 max-w-5xl">
      ...
      {!isLoading && tools.length === 0 && (
        <div className="border-2 border-dashed border-gray-200 rounded-xl p-12 text-center">
          <p className="text-gray-400 text-sm">No tools yet. Create your first fake tool.</p>
      ...
      <div className="grid gap-3">
        {tools.map(tool => (
          <ToolCard key={tool.id} tool={tool} onDelete={() => del.mutate(tool.id)} />
        ))}
      </div>
    </div>
  )
...
function ToolCard({ tool, onDelete }: { tool: Tool; onDelete: () => void }) {
